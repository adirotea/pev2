    ...
    <div :class="['text-left plan-node', {'detailed': showDetails, 'never-executed': isNeverExecuted, 'parallel': workersCount}]">
      ...
      <div class="plan-node-body card">
        <div class="card-body">
          ...
          <div>
            <span v-if="durationClass" :class="'p-0  d-inline-block mb-1 mr-1 text-nowrap alert ' + durationClass" title="Slow"><i class="fa fa-fw fa-clock-o"></i></span>
            <span v-if="costClass" :class="'p-0  d-inline-block mb-1 mr-1 text-nowrap alert ' + costClass" title="Cost is high"><i class="fa fa-fw fa-usd"></i></span>
        ...
        <div class="card-body tab-content" v-if="showDetails">
          <div class="tab-pane" :class="{'show active': activeTab === 'general' }">
            <!-- general -->
            <div v-if="node[nodeProps.ACTUAL_DURATION]">
              <b>Timing:</b>&nbsp;
              <span :class="'p-0 px-1 rounded alert ' + durationClass" v-html="formattedProp('ACTUAL_DURATION')"></span>
            ...
            <div>
              <b>Cost:</b> <span :class="'p-0 px-1 mr-1 alert ' + costClass">{{ formattedProp('ACTUAL_COST') }}</span> <span class="text-muted">(Total: {{ formattedProp('TOTAL_COST') }})</span>
            </div>
          ...
          <div class="tab-pane" :class="{'show active': activeTab === 'iobuffer' }">
            <div v-if="node[nodeProps.IO_READ_TIME] || node[nodeProps.IO_WRITE_TIME]" class="mb-2">
              <b>
                I/O Timings:
              </b>
              <span v-if="node[nodeProps.IO_READ_TIME]" class="ml-2">
                <b>Read:&nbsp;</b>
                {{ formattedProp('IO_READ_TIME') }}
              </span>
              <span v-if="node[nodeProps.IO_WRITE_TIME]" class="ml-2">
                <b>Write:&nbsp;</b>
                {{ formattedProp('IO_WRITE_TIME') }}
              </span>
            </div>
            <!-- iobuffer tab -->
            <b>
            ...
            <table class="table table-sm">
              ...
              <tr>
                <th>Shared</th>
                <td class="text-right">{{ formattedProp('SHARED_HIT_BLOCKS') || '-' }}</td>
                <td class="text-right">{{ formattedProp('SHARED_READ_BLOCKS') || '-' }}</td>
                <td class="text-right">{{ formattedProp('SHARED_DIRTIED_BLOCKS') || '-' }}</td>
                <td class="text-right">{{ formattedProp('SHARED_WRITTEN_BLOCKS') || '-' }}</td>
              </tr>
              <tr>
                <th>Temp</th>
                <td class="text-right">{{ formattedProp('TEMP_HIT_BLOCKS') || '-' }}</td>
                <td class="text-right">{{ formattedProp('TEMP_READ_BLOCKS') || '-' }}</td>
                <td class="text-right">{{ formattedProp('TEMP_DIRTIED_BLOCKS') || '-' }}</td>
                <td class="text-right">{{ formattedProp('TEMP_WRITTEN_BLOCKS') || '-' }}</td>
              </tr>
              <tr>
                <th>Local</th>
                <td class="text-right">{{ formattedProp('LOCAL_HIT_BLOCKS') || '-' }}</td>
                <td class="text-right">{{ formattedProp('LOCAL_READ_BLOCKS') || '-' }}</td>
                <td class="text-right">{{ formattedProp('LOCAL_DIRTIED_BLOCKS') || '-' }}</td>
                <td class="text-right">{{ formattedProp('LOCAL_WRITTEN_BLOCKS') || '-' }}</td>
              </tr>
            </table>
            <!-- iobuffer tab -->
          </div>
